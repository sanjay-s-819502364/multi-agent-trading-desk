...
2. Classify severity: Critical (data loss / pipeline outage / corrupted WFO results), High (feature broken, no workaround), Medium (degraded but usable), Low (cosmetic / edge case).
3. Note whether this looks like a known Python pitfall (None/attribute error, race condition, unawaited coroutine/blocked event loop, mutable default arg, stale/serialization mismatch, config/env mismatch, rate-limit error from the data provider API) — name it if so.
